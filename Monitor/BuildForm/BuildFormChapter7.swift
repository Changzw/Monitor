...
import UIKit
enum BuildFormChapter7{
  //MARK: - FormViewController

  static let hotspotForm: Form<Hotspot> =
    sections([
      section([
  ...
  static let buildPasswordForm: Form<Hotspot> =
    sections([
      section([
  ...
  static func uiSwitch<State>(keyPath: WritableKeyPath<State, Bool>) -> Rendered<State, UIView> {
    return { context in
      let toggle = UISwitch()
  ...
  static func textField<State>(keyPath: WritableKeyPath<State, String>) -> Rendered<State, UIView> {
    return { context in
  ...
  static func controlCell<State>(title: String, control: @escaping Rendered<State, UIView>, leftAligned: Bool = false) -> Rendered<State, FormCell> {
    return { context in
  ...
  static func detailTextCell<State>(title: String, keyPath: KeyPath<State, String>, form: @escaping Form<State>) -> Rendered<State, FormCell> {
    return { context in
      let cell = FormCell(style: .value1, reuseIdentifier: nil)
      cell.textLabel?.text = title
  ...
  static func section<State>(_ cells: [Rendered<State, FormCell>], footer keyPath: KeyPath<State, String?>? = nil) -> RenderedSection<State> {
    return { context in
  ...
  // todo DRY
  static func sections<State>(_ sections: [RenderedSection<State>]) -> Form<State> {
    return { context in
